...
class StockEnvV3(gym.Env):
    ...
    def _get_obs(self):
        ...
        for i, df in enumerate(self.dfs):
            ...
            # Normalize features and truncate
            obs.extend([
                round(price / 1000, 3),
                round(ma5 / 1000, 3),
                round(ma10 / 1000, 3),
    ...
    def step(self, actions):
        """
        Redid reward system to see better and faster results such as
        risk-adjusted metrics and per-stock rewards
        """
        done = False
        reward = 0
        ...
        # 1. Per-stock reward
        per_stock_rewards = stock_values - prev_stock_values
        per_stock_rewards = np.clip(per_stock_rewards / 100, -5, 5)
        ...
        if stock_values.sum() > 0:
            allocation_ratio = np.std(stock_values / stock_values.sum())
            reward -= allocation_ratio * 5

        reward = np.clip(reward, -10, 10)  # final reward clipping
